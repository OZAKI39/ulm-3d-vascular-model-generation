...
<img alt="1787394701972" src="image/文本记录/1787394701972.png" width="937" height="743">
...
每次运行均建立独立的时间标识目录，保存内容可以按用途分为三类。第一类是可追溯性证据，包括本次使用的完整 YAML 源配置、解析后的有效运行配置、样本清单、完整 `reference_swc`、所选 `analysis_swc`、原始分量审计记录、`REFERENCE_ONLY` 节点清单、可选体数据与 Mask QC、全局节点和边标识，以及局部—全局映射。借助这些信息，任意代表性 ROI 均可返回源样本和分析网络中的原始位置，同时能够确认哪些参考分量未进入当前分析但仍被完整保留。
...
- 将入口文件更新为 `swc_roi_generate.py`，并把原命令行超参数统一迁移到 `configs/swc_roi_generate.yaml`；运行结果同时保存源 YAML 和解析后的有效配置，便于核对参数来源。
...
在项目目录 `ulm_3D_vascular` 下运行。当前入口文件为 `swc_roi_generate.py`，全部处理和采样参数集中保存在 `configs/swc_roi_generate.yaml`，入口脚本不再逐项接收命令行超参数。配置中的相对路径均以项目目录 `ulm_3D_vascular` 为基准，因此配置文件放在 `configs` 子目录不会改变输入与输出路径的含义。

使用当前正式配置时运行：

```powershell
D:\anaconda3\envs\pmp\python.exe swc_roi_generate.py configs\swc_roi_generate.yaml
```
...
YAML 按用途分为 `paths`、`pipeline`、`coordinates`、`preprocessing`、`exports`、`visualization`、`sampling` 和 `runtime`。当前文件明确记录输入数据目录、输出目录、样本编号、物理间距、中心线处理、导出格式、可视化方式、ROI 尺寸、锚点间距、特征组成、聚类数量和代表选择约束。程序读取配置时会检查字段名称、数据类型、三维数组长度和允许取值；无法识别的字段或不合法数值会在数据处理开始前终止运行，避免因 YAML 键名拼写错误而静默使用其他参数。

当前配置中 `preprocessing.smooth_centerlines: false` 表示不启用中心线平滑，`preprocessing.resample_step_um: 1.0` 只规定层级分支派生几何的采样间距，不修改或替换源 SWC 节点。`visualization.figure2a.show_gui: false` 阻止交互窗口弹出，但仍生成屏幕外验收图；将其改为 `true` 后可打开双视窗界面。界面中 `A` 键显示全部候选 ROI，`R` 或 `S` 键返回已选代表，`C` 键依次查看不同聚类，关闭窗口后程序才结束。若只需要全局 SWC 预处理和层级图，可把 `sampling.enabled` 改为 `false`。若只想查看入口用法，可运行 `D:\anaconda3\envs\pmp\python.exe swc_roi_generate.py --help`。

为保证复现时采用的参数文件不随项目后续修改而丢失，程序会把本次读取的原始 YAML 分别复制到全局处理结果和 ROI 采样结果中；解析后的有效配置仍由原有运行配置文件保存。正式实验应为每次参数变化保留独立 YAML，运行时把该文件路径作为唯一位置参数传入。

### A.2 输出目录与内容

一次完整运行会建立两个相互关联、但分别带有时间标识的结果目录。程序结束前会在终端打印二者的实际路径、验收报告和运行状态。

第一类目录保存全局 SWC 预处理和层级图结果：

```text
outputs/rodent_vasculature/
└── all_run_YYYYMMDD_HHMMSS/
    ├── inventory/                 # 数据清单与编目汇总
    ├── samples/
    │   └── fMOST_0_5_6_0_0_6_0001_02_01/
    │       ├── normalized/        # 完整 reference_swc、analysis_swc 与可选显示体数据
    │       ├── swc_analysis_preprocessing/
    │       │                      # 连通分量、REFERENCE_ONLY 节点和可选 Mask QC
    │       ├── graphs/            # 节点、父子边、聚合分支及层级图
    │       ├── visualizations/     # 方向、拓扑和 Figure 2(a) 风格验收图
    │       ├── preprocess_manifest.json
    │       ├── preprocess_acceptance.json
    │       ├── graph_summary.json
    │       ├── graph_acceptance.json
    │       └── acceptance_report.html
    ├── reports/                   # 样本汇总与失败记录
    ├── normalized_index.json      # 后续层级建图和 ROI 采样的来源索引
    ├── source_swc_roi_generate.yaml
    │                              # 本次运行读取的完整 YAML 原文件
    ├── run_config.json
    ├── pipeline.log
    ├── acceptance.json
...
`source_swc_roi_generate.yaml` 是本次运行实际读取的完整源配置，`run_config.json` 保存解析路径并转换为有效类型后的全局处理配置。`normalized` 目录中的 `reference_swc_complete.swc/.npz` 保存完整参考 SWC，`analysis_swc_single_component.swc/.npz` 保存按既定规则选中的原始连通分量。`swc_analysis_preprocessing` 记录所选分量、未进入分析的参考节点以及可选图像和 Mask 的配准检查。`graphs` 目录保存 `directed_nodes.csv`、`source_parent_to_current_edges.csv`、`directed_branches.csv`、层级图文件和分支几何；其中的方向表示 SWC 父节点到当前节点的记录关系，不能解释为已经测得的血流方向。

第二类目录保存真实连通 ROI 的候选、特征、聚类和代表选择结果：

```text
outputs/sampling/
└── YYYYMMDD_HHMMSS_radius_plus_structure_k5/
    ├── logs/              # ROI 采样运行日志
    ├── config/            # 完整源 YAML 与解析后的本次采样参数
    ├── manifests/         # 全局边、候选 ROI、已选 ROI、裁切端口和聚类汇总
    ├── features/          # 每个 ROI 的半径—结构特征及稳健尺度变换状态
    ├── clustering/        # 聚类分配、聚类中心和候选 K 值比较
    ├── figures/           # 候选分布、代表选择、特征分布和交互层预览
    ├── roi_previews/      # 已选代表性 ROI 的独立预览
    ├── roi_library/       # 每个有效真实 ROI 的完整几何、半径和局部—全局映射
    ├── comparison/        # 不同特征组合的对照结果
    └── report/            # 采样汇总、来源检查与最终状态
```

`config/source_swc_roi_generate.yaml` 是与全局处理结果相同的完整源配置副本，`config/sampling_config.yaml` 保存解析后的有效采样配置。`manifests/candidate_rois.csv` 保存有效候选区域，`manifests/selected_rois.csv` 保存最终代表区域，`manifests/cut_ports.csv` 记录人为裁切端口。`features/roi_features.csv` 保存半径和结构特征。`roi_library` 中每个 NPZ 文件对应一个真实连通 ROI，包含节点位置、半径、局部边、全局标识映射、源图终端和 CUT_PORT；该目录同时保存候选与代表的底层几何，是否为代表应以 `selected_rois.csv` 和文件内部的选择信息为准。`report/sampling_summary.json` 汇总候选数、拒绝原因、聚类、代表数量、完整性检查和运行时间，`report/run_status.json` 给出采样阶段的最终状态。

程序返回状态 0 表示所请求的全局处理已经完成，且启用 ROI 采样时采样验收也通过；状态 2 表示流程完成了诊断，但全局或采样验收为失败；状态 1 表示输入读取、处理、采样或可视化过程中发生异常。由于两个结果目录各自使用创建时刻命名，不应根据目录名猜测对应关系；复核时应以终端打印的 `Run directory`、`Sampling run directory`，以及各目录中的配置、清单和汇总文件为准。
