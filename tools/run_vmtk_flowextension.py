...
def _run_extension(request: dict[str, object]) -> dict[str, object]:
    ...
    if str(parameters["extension_mode"]) != "boundarynormal":
    ...
    return {
        **_runtime(),
        ...
        "extension_mode_effective": "boundarynormal",
        "official_direction_api": "SetExtensionModeToUseNormalToBoundary",
...
def _run_cap(request: dict[str, object]) -> dict[str, object]:
    """Cap the accepted cross-seam OPEN surface without remeshing."""

    raw = _read_vtp(str(request["open_vtp"]))
